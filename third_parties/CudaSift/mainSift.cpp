...
void MatchAll(SiftData &siftData1, SiftData &siftData2, float *homography) {
  ...
  for (int i = 0; i < numPts1; i++) {
    ...
    std::cout << i << ":" << sift1[i].scale << ":" << (int)sift1[i].orientation
              << " " << sift1[i].xpos << " " << sift1[i].ypos << std::endl;
    bool found = false;
    for (int j = 0; j < numPts2; j++) {
      ...
      if (err < 100.0f || j == sift1[i].match) {  // 100.0
        ...
        std::cout << j << ":" << sum << ":" << (int)sqrt(err) << ":"
                  ...
                  << sift2[j].xpos << " " << sift2[j].ypos << " " << (int)dx
                  << " " << (int)dy << std::endl;
      }
    }
    std::cout << std::endl;
...
void PrintMatchData(SiftData &siftData1, SiftData &siftData2, CudaImage &img) {
  ...
  for (int j = 0; j < numPts; j++) {
    ...
    int s = std::min(
        x, std::min(
               y, std::min(w - x - 2,
                           std::min(h - y - 2, (int)(1.41 * sift1[j].scale)))));
    int p = y * w + x;
    p += (w + 1);
    for (int k = 0; k < s; k++)
